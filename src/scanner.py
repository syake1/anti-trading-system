...
RESULT_COLUMNS = ["シグナル日", "コード", "会社名", "市場", "現在値", "前日比", "直近3日騰落率", "直近5日騰落率",
                  ...
                  "アンチ判定", "買い・売り", "シグナル種別", "シグナル数", "自社株買い比率", "損切り候補", "利確候補", "RR", "判定理由", "除外理由", "Yahoo Financeリンク", *MARGIN_COLUMNS, *MARGIN_SCORE_COLUMNS]
...
def run(notify: bool = True) -> Path:
    ...
    result = apply_margin_scoring(enrich_margin(result, config, ROOT)).reindex(columns=RESULT_COLUMNS)

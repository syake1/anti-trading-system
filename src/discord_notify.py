...
def candidate_message(row: dict) -> str:
    ...
    count = int(row.get("シグナル数", 1))
    label = "TRIPLE SIGNAL" if count >= 3 else "DOUBLE SIGNAL" if count >= 2 else "逆張り"
    return (f'{icon} {label} {row["ランク"]}ランク\n\n'
            ...
            f'シグナル：{row.get("シグナル種別", "逆張り")}\n急騰判定：{row.get("除外理由") or "なし"}\n損切り：{row["損切り候補"]:,.1f}円\n'
